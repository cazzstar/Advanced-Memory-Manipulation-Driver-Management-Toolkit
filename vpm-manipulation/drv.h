...
public:
	...
	c_pmx( )
	{
		//auto m_file = to_file( );
		//if ( !m_file )
		//	std::printf( " [file] -> failed to convert.\n" );

		//auto m_start_service = register_and_start( );
		//if ( !m_start_service )
	...
	{
		CloseHandle( hHandle );
		auto status = stop_and_delete( );
		if ( !status )
		{
			std::printf( " [service] -> failed to stop.\n" );
		}
	} 
}; inline auto m_physical = std::make_shared< c_pmx >( );
